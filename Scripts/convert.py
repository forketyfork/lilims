        ...
        try:
            ...
            if hasattr(tensor, 'tensor_type'):
                ...
                if tensor_type in [gguf.GGMLQuantizationType.F32, gguf.GGMLQuantizationType.F16]:
                    ...
                    logging.warning(f"Unsupported tensor type {tensor_type} for {tensor_name}, attempting raw conversion")
                    try:
                        data = np.array(tensor.data, dtype=np.float16)
            ...
            if data is not None:
                ...
                logging.debug(f"Loaded tensor {tensor_name}: shape {data.shape}, type: {getattr(tensor, 'tensor_type', 'unknown')}")
            else:
